...
function updateTodayInfo() {
    ...
    if (todayEntry) {
        // Add tasks from tasks list (Goal #2)
        if (state.tasks[todayEntry.date] && state.tasks[todayEntry.date].length > 0) {
            state.tasks[todayEntry.date].forEach(task => {
                const li = document.createElement('li');
                li.textContent = task.text;
                elements.todayTasks.appendChild(li);
            });
        } else if (todayEntry.eventsNotes) {
            // Fallback to old method of extracting tasks from events/notes
